// Имя файла: F:/app/D&D/app/src/main/java/com/dnd/app\ui\screens\character_creator\InventoryHandler.kt
// --- НАЧАЛО ФАЙЛА ---
...
class InventoryHandler @Inject constructor(
    ...
    fun setInventoryMode(mode: InventoryMode) {
        scope.launch {
            ...
            val baseDraft = currentState.draft.copy(baseInfo = currentState.draft.baseInfo.copy(inventorySelections = emptyMap()))

            if (mode == InventoryMode.BUY_WITH_GOLD) {
                val firstClassIdx = baseDraft.levelStack.firstOrNull()?.classIndex ?: return@launch


                val bgGold = baseDraft.baseInfo.startingGold


                val classGold = getStartingGoldForClass(firstClassIdx).gp


                val totalStartingGold = Money(gp = bgGold + classGold)

                _uiState.update { it.copy(
                    draft = baseDraft,
                    inventoryMode = mode,
                    initialGold = totalStartingGold,
                    remainingGold = totalStartingGold,
