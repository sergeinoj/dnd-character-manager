...
class CharacterCreatorViewModel @Inject constructor(
    ...
    fun setInventoryMode(mode: InventoryMode) {
        viewModelScope.launch {
            ...
            // Полная очистка инвентаря при смене режима
            ...
            if (mode == InventoryMode.BUY_WITH_GOLD) {
                ...
                val newDraft = baseDraft.copy(baseInfo = baseDraft.baseInfo.copy(staticEquipment = bgEquipment)) // Только от предыстории
                _uiState.update { it.copy(
    ...
    fun selectShopCategory(category: ShopCategory) {
        viewModelScope.launch {
            // Сначала проверяем, есть ли у этой категории подкатегории
    ...
    fun goBackInShop() {
        viewModelScope.launch {
            if (categoryStack.isNotEmpty()) {
                categoryStack.pop() // Удаляем текущую
            }

            if (categoryStack.isEmpty()) {
                loadRootShopCategories() // Вернулись в корень
            } else {
                val parent = categoryStack.peek()
                // Загружаем дочерние категории родителя
                parent?.let {
                    val children = libraryRepository.getChildShopCategories(it.index)
                    _uiState.update { state -> state.copy(
                        shopCategories = children, shopView = ShopView.CATEGORIES,
                        currentShopTitle = it.name
                    )}
                } ?: loadRootShopCategories() // Fallback на всякий случай
            }
        }
    }
    ...
    fun searchShop(query: String) {
        ...
        searchJob = viewModelScope.launch {
            delay(300) // Debounce
            if (query.isBlank()) {
                // Если мы в режиме просмотра товаров, возвращаемся к списку товаров категории
                if (_uiState.value.shopView == ShopView.ITEMS && categoryStack.isNotEmpty()) {
                    categoryStack.peek()?.let { selectShopCategory(it) }
                } else { // Иначе - к списку категорий
                    goBackInShop()
                }
                return@launch
            }
    ...
    private fun getStartingGoldForClass(classIndex: String): Money {
        return when (classIndex) {
            "barbarian" -> Money(gp = Random.nextInt(2, 9) * 10) // 2d4 * 10
            "bard" -> Money(gp = Random.nextInt(5, 21) * 10)      // 5d4 * 10
            "cleric" -> Money(gp = Random.nextInt(5, 21) * 10)    // 5d4 * 10
            "druid" -> Money(gp = Random.nextInt(2, 9) * 10)       // 2d4 * 10
            "fighter" -> Money(gp = Random.nextInt(5, 21) * 10)   // 5d4 * 10
            "monk" -> Money(gp = Random.nextInt(5, 21))           // 5d4
            "paladin" -> Money(gp = Random.nextInt(5, 21) * 10)   // 5d4 * 10
            "ranger" -> Money(gp = Random.nextInt(5, 21) * 10)    // 5d4 * 10
            "rogue" -> Money(gp = Random.nextInt(4, 17) * 10)     // 4d4 * 10
            "sorcerer" -> Money(gp = Random.nextInt(3, 13) * 10)  // 3d4 * 10
            "warlock" -> Money(gp = Random.nextInt(4, 17) * 10)   // 4d4 * 10
            "wizard" -> Money(gp = Random.nextInt(4, 17) * 10)    // 4d4 * 10
            else -> Money(gp = 100) // Fallback
        }
    }
