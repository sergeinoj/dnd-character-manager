...
class ClassProgressionUseCase @Inject constructor(
    ...
    suspend fun generateInitialFeatures(classEntity: ClassEntity): List<Feature> {
        val features = mutableListOf<Feature>()
        features.addAll(createInitialProficiencyFeatures(classEntity))
        features.addAll(createInitialSpellFeatures(classEntity))
        // [НОВЫЙ ВЫЗОВ v1.25]
    ...
    private suspend fun createInitialSpellFeatures(classEntity: ClassEntity): List<Feature> {
