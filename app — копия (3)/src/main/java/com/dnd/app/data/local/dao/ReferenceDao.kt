...
interface ReferenceDao {
    ...
    @Query("SELECT * FROM features WHERE index_name LIKE 'feat-%' ORDER BY name ASC")
    ...
    // --- СНАРЯЖЕНИЕ (v1.25 SHOP UPDATE) ---
    // Weapons
    @Query("SELECT * FROM weapons WHERE index_name IN (:indexes) ORDER BY label ASC")
