-- D&D 5.1 Absolute Schema v1.25 (Shop Ready)
PRAGMA journal_mode = WAL;
PRAGMA foreign_keys = ON;

-- ================================================================
-- 1. МАГАЗИН И ИНВЕНТАРЬ (SHOP & INVENTORY) - [UPDATED v1.25]
-- ================================================================

-- КАТЕГОРИИ СНАРЯЖЕНИЯ
-- Добавлен parent_index для вложенности (например, 'weapons' -> 'martial-weapons')
...
-- ОРУЖИЕ
-- Добавлены cost_json (исходник) и cost_cp (цена в медных монетах для сортировки)
...
-- ОБЩЕЕ СНАРЯЖЕНИЕ
-- Добавлен cost_cp (cost_json уже присутствовал)
...
-- МАГИЧЕСКИЕ ПРЕДМЕТЫ
-- Добавлены cost_json и cost_cp (для расчетной стоимости от редкости)
...
-- ИНДЕКСЫ ДЛЯ МАГАЗИНА (SHOP PERFORMANCE)
...
-- ================================================================
-- 2. ОПЦИИ ПЕРСОНАЖА И ПРАВИЛА (CORE RULES) - [UNCHANGED v1.24]
-- ================================================================
...
CREATE TABLE IF NOT EXISTS subclasses (id INTEGER PRIMARY KEY AUTOINCREMENT, index_name TEXT NOT NULL UNIQUE, class_index TEXT NOT NULL, name TEXT NOT NULL, subclass_flavor TEXT, description TEXT, spells_json TEXT, subclass_levels_url TEXT);
