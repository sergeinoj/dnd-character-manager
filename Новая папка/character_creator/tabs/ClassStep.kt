...
fun ClassStep(
    ...
    LazyColumn(
        ...
        if (selClass != null) {
            ...
            classStepData.subclassChoiceFeature?.let { feature ->
                if (classStepData.availableSubclasses.isNotEmpty()) {
                    item(key = "subclass_choice") {
                        FlatWizardSection(title = feature.name) {
                            Column {
                                ...
                                val subOpts = classStepData.availableSubclasses.map { ChoiceOption(it.index, it.name) }
                                SmartDropdown(
                                    options = subOpts,
            ...
            items(classStepData.classFeatures.sortedBy { it.priority }, key = { "base_feat_${it.index}" }) { feature ->
            ...
            classStepData.aggregatedSpellFeature?.let { feature ->
