...
object WizardUiConfig {
    ...
    // === ВЫБОР СНАРЯЖЕНИЯ (INVENTORY STEP) ===

    // --- Переключатель режима (Стандартный / Покупка) ---
    ...
    // --- Заголовок группы (например, "Снаряжение: Бард (вариант 1)") ---
    ...
    /** Вертикальный отступ между группами снаряжения (например, между "Снаряжение: Бард 1" и "Снаряжение: Бард 2"). */
    val EQUIP_FEATURE_RENDERER_SPACING: Dp = 8.dp
    /** Горизонтальный отступ между карточками выбора в одном ряду. */
    val EQUIP_CHOICE_ROW_SPACING: Dp = 8.dp

    // --- Карточки выбора (общие стили) ---
    ...
    val EQUIP_SELECT_BTN_ACTIVE_BG: Color = Color(0xFF424242) // Используется и для рамки выбранной карточки
