...
class CharacterAssembler @Inject constructor(
    ...
    suspend fun assemble(draft: DraftCharacter): CharacterDomain {
        ...
        if (background != null) {
            ...
            allEquipmentIndexes.addAll(background.staticEquipment)
        }
        ...
        draft.levelStack.forEachIndexed { index, step ->
            ...
            if (index == 0) {
                ...
                // Стартовые владения 1-го уровня
                val classEntity = repository.getClassEntityByIndex(step.classIndex)
                classEntity?.proficienciesJson?.let { raw -> try { json.decodeFromString<List<ReferenceJson>>(raw).forEach { skillProficiencies[it.index] = 1 } } catch(e: Exception){} }
                classEntity?.savingThrowsJson?.let { raw -> try { json.decodeFromString<List<ReferenceJson>>(raw).forEach { skillProficiencies["saving-throw-${it.index.lowercase()}"] = 1 } } catch(e: Exception){} }
                classEntity?.startingEquipmentJson?.let { raw -> try { json.decodeFromString<List<ReferenceJson>>(raw).forEach { allEquipmentIndexes.add(it.index) } } catch(e: Exception){} }
            } else {
                hpMax += step.hpIncrease + conMod
            }

            // [КРИТИЧЕСКИЙ ФИКС] Получение фич уровня (включая Domain Spells)
        ...
        // [ФИНАЛЬНЫЙ МАГИЧЕСКИЙ АККОРД] Слияние всех источников магии
        val allSpellsDb = repository.getAllSpells().first()

        // 1. Пришитые (embedded) заклинания из всех собранных фич (Раса, Домен, Клятва)
        val guaranteedSpellIds = features.flatMap { it.embeddedSpells }.map { it.id }.distinct()

        // 2. Заклинания, выбранные игроком в Мастере (Choices)
        ...
        results.filter { !it.key.contains("expertise", true) }.forEach { (_, result) ->
            when (result) {
                ...
                is ChoiceResult.SelectedOptions -> result.items.forEach { id ->
                    ...
                    // На всякий случай ловим заклинания из SelectedOptions
